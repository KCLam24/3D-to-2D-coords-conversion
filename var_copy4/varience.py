...
variance_a = variance_b = [0.1, 0.5, 1]
mean = [1, 1]
...
for i in range(len(variance_a)):
    for j in range(len(variance_b)):
        ...
        ellips_img_train = system_maker2(12500, 30, 1, 100, 2, (64, 64),d, False)
        
        # Generate testing images with different parameters
        sphere_img_test = system_maker(2500, 30, 1, 100, 2, (64, 64),d)
        ellips_img_test = system_maker2(2500, 30, 1, 100, 2,(64, 64),d, False)
    
        end = timer()
        
        print(f'\nvariance a:{variance_a[i]}, variance b:{variance_b[j]}, mean: {mean}, covarient: {cov_ab}')
        ...
        plot_loss_curves(model_results, f'Loss_a{variance_a[i]}_b{variance_b[j]}')
