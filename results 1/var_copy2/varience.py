...
def system_maker(no_of_systems ,max_spheres, max_sphere_size, no_of_points, no_of_projections, image_res):
    ...
    for _ in range(no_of_systems):
        ...
        for _ in range(no_of_spheres):
            ...
            a = a + (np.random.rand(1,3) * np.random.randint(10,40)) # translate sphere around
...
def system_maker2(no_of_systems ,max_ellipsoids, max_ellipsoid_size, no_of_points, no_of_projections, image_res):
    ...
    for _ in range(no_of_systems):
        ...
        for _ in range(no_of_ellipsoids):
            a = ellipsoid(radius=max_ellipsoid_size, num_points=no_of_points, mean=mean, cov=cov, log_normal=True)
...
for i in range(len(a)):
    for j in range(len(b)):
        ...
        sphere_img2 = system_maker(10000,30,1,100,2,(64,64))
        ellips_img2 = system_maker2(10000,30,1,100,2,(64,64))
        #end = time.time()
        ...
        plot_images(images_cont[40],images_cont[10000],images_cont[31068],images_cont[32091],f'varlog_a{variance_a}_b{variance_b}')
        print(f'variance a:{variance_a}, variance b:{variance_b}, mean: {mean}, cov: {cov}')
        ...
        plot_loss_curves(model_results, f'Losslog_a{variance_a}_b{variance_b}')
