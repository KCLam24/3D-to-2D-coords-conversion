...
def generate_ratios(mean, cov, distribution='gaussian'):
    if distribution == 'lognormal':
        # Use the mean and standard deviation (sqrt of variance) to create log-normal distribution
        ...
        a_c = powerlaw.rvs(a=mean[0], scale=2)
        b_c = powerlaw.rvs(a=mean[1], scale=2)
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=3, stride=1, padding=1, padding_mode='circular')
...
def system_maker2(no_of_systems ,max_ellipsoids, max_ellipsoid_size, no_of_points, no_of_projections, image_res, distance, distribution='gaussian'):
    bw_img_all = []
    grid_img_all = []
    for _ in range(no_of_systems):
        systems = []
        ...
        for _ in range(no_of_ellipsoids):
            a = ellipsoid(radius=max_ellipsoid_size, num_points=no_of_points, mean=mean, cov=cov, distribution=distribution)
...
d = 50
points_no = [1,3, 5, 10, 30, 60, 80]
image_name = 'dens_img_points'
loss_name = 'dens_loss_points'
for p in points_no:
    start = timer()
    '''system_maker(no_of_systems ,max_spheres, max_sphere_size, no_of_points, no_of_projections, image_res)'''
    
    # Generate training images
    sphere_img_train = system_maker(15000, 50, 2, p, 2, (64, 64),d)
    ellips_img_train = system_maker2(15000, 50, 2, p, 2, (64, 64),d, 'powerlaw')
    
    # Generate testing images with different parameters
    sphere_img_test = system_maker(3750, 50, 2, p, 2, (64, 64),d)
    ellips_img_test = system_maker2(3750, 50, 2, p, 2,(64, 64),d, 'powerlaw')

    end = timer()
    
    print(f'\nvariance a:{variance_a}, variance b:{variance_b}, mean: {mean}, covarient: {cov_ab}, distance: {d}, no. points={p}')
    ...
    plot_images(images_train[4],images_train[5000],images_train[40000],images_train[56792],f'{image_name}_{p}')
    ...
    plot_loss_curves(model_results, f'{loss_name}_{p}')
