...
variance_a = variance_b = [0.1, 0.5, 1, 2]
mean = [1, 1]
cov = [[variance_a, cov_ab], 
       [cov_ab, variance_b]]
d = 20
image_name = 'var_exp_img'
loss_name = 'var_exp_loss'
...
for i in range(len(variance_a)):
    for j in range(len(variance_b)):
        ...
        sphere_img_train = system_maker(12500, 30, 1, 100, 2, (64, 64),d)
        ellips_img_train = system_maker2(12500, 30, 1, 100, 2, (64, 64),d, 'exponential')
        
        # Generate testing images with different parameters
        sphere_img_test = system_maker(2500, 30, 1, 100, 2, (64, 64),d)
        ellips_img_test = system_maker2(2500, 30, 1, 100, 2,(64, 64),d, 'exponential')
    
        end = timer()
        
        print(f'\nvariance_a:{variance_a[i]}, variance_b:{variance_b[j]}, mean: {mean}, covarient: {cov_ab}, distance: {d}')
        ...
        plot_images(images_train[4],images_train[1000],images_train[30000],images_train[45000],f'{image_name}_a{variance_a[i]}_b{variance_b[j]}')
        ...
        plot_loss_curves(model_results, f'{loss_name}_a{variance_a[i]}_b{variance_b[j]}')
